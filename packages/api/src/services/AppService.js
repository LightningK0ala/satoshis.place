...
class AppService extends EventEmitter {
  ...
  */
  ...
  async onGetSettings (_, sid) {
    const result = await SettingsModel.findOne({}).select('-createdAt -updatedAt -_id -__v').lean()
  ...
  async onNewOrder (data, sid) {
    ...
    if (hasError) {
      ...
      */
      ...
      |--------------------------------------------------
      */
      const paymentRequest = await LightningService.newInvoice(data.length, settings)
  ...
  async onNewInvoiceResult ({ data, paymentRequest }, sid) {
    ...
    const orderImage = encodeRGBAArrayToPngBase64(rgbaOrder, config.BOARD_LENGTH)
  ...
  async onGetLatestPixels (data, sid) {
    ...
    const result = await PixelModel.findOne()
      .select('pixels -_id')
      .lean()
  ...
  */
  ...
  async updatePixels (orderPixels) {
    ...
    log.info(`Decoding base64 to pixel array OK`, phrtime(process.hrtime(nTime)))
    log.info(`Looping through pixel array`)
    nTime = process.hrtime()
    // Update pixels
    const pixelArray = updateNonOpaqueRGBAArray(orderPixelArray, basePixelArray)
    ...
    log.info(`Encoding pixel array OK, total time was`, phrtime(process.hrtime(nTime)))
