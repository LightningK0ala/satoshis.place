...
const isAlreadyPaintedWithCurrentColor = (coordsStr, color) =>
  paintedPixels[coordsStr] && color !== paintedPixels[coordsStr];
...
const paintPixel = (coordsToDraw, coordsStr, color, size, isPreviewModeOn) => {
  ...
  graphics.drawRect(coordsToDraw.x, coordsToDraw.y, size, size);
  graphics.endFill();
  ...
  .actions((self) => ({
    ...
    resizeCanvas: (width, height) => {
      ...
      if (self.isInitialized) {
        ...
        viewport &&
          viewport.resize(width, height, self.boardLength, self.boardLength);
      }
    },
    /**
     * Scale Board
    ...
    centerViewport: () => {
      viewport &&
        viewport.moveCenter(self.boardLength / 2, self.boardLength / 2);
    },
    /**
     * New order
     */
    ...
    setClientWidthHeight: (cwh) => {
      clientWidthHeight = cwh;
    },
    /**
     * Stats updated
     */
    ...
    onOrderSettled: ({ data, error }) => {
      ...
      self.updateCanvas(data.image, () => {
        ...
        self.addHudMessage({
          ...
          } painted ${data.pixelsPaintedCount} pixel${
            data.pixelsPaintedCount > 1 ? "s" : ""
          }.`,
    ...
    initPixi: () => {
      ...
      viewport.decelerate({ friction: 0.8 }).pinch();
      ...
      self.resizeCanvas(clientWidthHeight.width, clientWidthHeight.height);
      // Center the viewport
      self.centerViewport();
    ...
    registerBaseSpriteListeners: (baseSprite) => {
      baseSprite.on("pointerdown", (e) => {
        const c = self.validateAndReturnCoords(
          e.data.global.x,
          e.data.global.y
        );
        ...
        // Drawing
        if (self.isDrawMode) {
          const coordsToDrawArray = getThickBrushCoordinates(
            self.brushSize,
            c.normalizedCoords,
            self.pixelSize
          );
          coordsToDrawArray.map((_c) => {
            paintPixel(
              _c.coordsToDraw,
              _c.coordsStr,
              self.paintColor,
      ...
      baseSprite.on("pointermove", (e) => {
        ...
        // Check validation result
        if (!c) return;
        if (self.isDrawMode || self.isEraseMode) {
          self.hoverPixel(c.coordsToDraw);
        }
        if (self.isDrawMode && self.mouseDown) {
          ...
          if (lastDrawnPoint.normalizedCoords) {
            ...
            const xDiff =
              c.normalizedCoords.x - lastDrawnPoint.normalizedCoords.x;
            const yDiff =
              c.normalizedCoords.y - lastDrawnPoint.normalizedCoords.y;
            ...
            if (xDiffAbs > 1 || yDiffAbs > 1) {
              ...
              if (xDiffAbs > yDiffAbs) {
                ...
                for (let dy = 0; dy <= yDiffAbs; dy++) {
                  let y =
                    lastDrawnPoint.normalizedCoords.y + (yDiff > 0 ? dy : -dy);
                  let x = m * y + b;
                  x = Math.floor(x);
                  const coordsToDrawArray = getThickBrushCoordinates(
                    self.brushSize,
        ...
        self.isEraseMode &&
          self.mouseDown &&
          self.erasePixel(c.coordsToDraw, c.coordsStr);
      });
    ...
    zoomIn: (pctOverride) =>
      ...
      viewport.zoomPercent(pctOverride || ZOOM_IN_BY_PERCENT, true),
    /**
     * Zoom out the viewport
     */
    zoomOut: (pctOverride) =>
      viewport.transform.scale.x > 0.05 &&
      viewport.zoomPercent(-pctOverride || -ZOOM_OUT_BY_PERCENT, true),
    /**
     * Clear drawing
     */
    clearDrawing: () => {
